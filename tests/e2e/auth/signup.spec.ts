...
test.describe('User Signup and Band Creation', () => {
  ...
  test('band creation handles RLS policies correctly', async ({ page }) => {
    ...
    page.on('console', msg => {
      const text = msg.text();
      if (text.includes('RLS') || text.includes('policy') || text.includes('permission')) {
        rlsErrors.push(text);
      }
    });
  ...
  test('new user sees correct onboarding flow', async ({ page }) => {
    ...
    await expect(page.locator('text=Create Your First Band').first()).toBeVisible();
    await expect(page.locator('text=Join an Existing Band').first()).toBeVisible();
