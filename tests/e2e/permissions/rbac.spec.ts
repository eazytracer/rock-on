...
test.describe('Role-Based Access Control (RBAC)', () => {
  ...
  test('admin has full access to all band features', async ({ page }) => {
    ...
    for (const item of navItems) {
      const navLink = page.locator(`a:has-text("${item}"), [href*="${item.toLowerCase().replace(' ', '-')}"]`).first();
  ...
  test('regular member has appropriate permissions', async ({ browser }) => {
    ...
    const addSongButton = memberPage.locator('button:has-text("Add Song")').first();
  ...
  test('regular member cannot remove other members', async ({ browser }) => {
    ...
    if (rowVisible) {
      ...
      const removeButton = member1Page.locator('button:has-text("Remove"), button:has-text("Remove from Band")').first();
  ...
  test('admin can remove regular members', async ({ browser }) => {
    ...
    const isVisible = await memberRow.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (isVisible) {
      ...
      const removeButton = adminPage.locator('button:has-text("Remove"), button:has-text("Remove from Band")').first();
      const canRemove = await removeButton.isVisible().catch(() => false);
  ...
  test('owner has all admin permissions', async ({ page }) => {
    ...
    const ownerRow = page.locator(`[data-testid="member-row-${owner.email}"]`).first();
  ...
  test('member cannot promote themselves to admin', async ({ browser }) => {
    ...
    const memberRow = memberPage.locator(`[data-testid="member-row-${member.email}"]`).first();
    await memberRow.click();
    await memberPage.waitForTimeout(500);

    // Should not see promote button or role selector
    const promoteButton = memberPage.locator('button:has-text("Promote"), button:has-text("Make Admin")').first();
  ...
  test('all members can add and edit songs', async ({ browser }) => {
    ...
    if (canAdd) {
      ...
      await memberPage.locator('button:has-text("Save")').first().click();
      await memberPage.waitForTimeout(2000);

      // Verify song was added
      await expect(memberPage.locator('text=Member Song').first()).toBeVisible({ timeout: 5000 });
      console.log('✓ Members can add songs');
    }
    ...
    const memberSong = await adminPage.locator('text=Member Song').first().isVisible().catch(() => false);
    if (memberSong) {
