...
test.describe('Setlist and Show Sync Verification', () => {
  ...
  test('creating a setlist syncs to Supabase without errors', async ({ page }) => {
    ...
    await page.waitForSelector('h1:has-text("Setlists"), [data-testid="setlist-empty-state"], [data-testid="create-setlist-button"]', { timeout: 15000 });

    // Click Add/Create Setlist button
    const addButton = page.locator('[data-testid="create-setlist-button"]').first();
    await expect(addButton).toBeVisible({ timeout: 10000 });
    await addButton.click();
    await page.waitForTimeout(500);

    // Fill in the setlist name
    const nameInput = page.locator('input[name="name"], [data-testid="setlist-name-input"]').first();
    ...
    console.log(`✅ Setlist "${setlistName}" verified in Supabase with ID: ${setlists![0].id}`);
  });
  ...
  test('creating a show syncs to Supabase without errors', async ({ page }) => {
    ...
    await page.waitForSelector('h1:has-text("Shows"), [data-testid="create-show-button"], button:has-text("Add Show")', { timeout: 15000 });

    // Click Add/Create Show button
    const addButton = page.locator('[data-testid="create-show-button"], button:has-text("Add Show")').first();
    await expect(addButton).toBeVisible({ timeout: 10000 });
    await addButton.click();
    await page.waitForTimeout(500);

    // Fill in show details
    const nameInput = page.locator('input[name="name"], [data-testid="show-name-input"]').first();
    await expect(nameInput).toBeVisible({ timeout: 5000 });
    await nameInput.fill(showName);

    // Fill venue
    const venueInput = page.locator('input[name="venue"], [data-testid="show-venue-input"]').first();
    if (await venueInput.isVisible()) {
      await venueInput.fill(showVenue);
    }

    // Fill location (this is the field that was missing from schema)
    const locationInput = page.locator('input[name="location"], [data-testid="show-location-input"]').first();
    if (await locationInput.isVisible()) {
      await locationInput.fill(showLocation);
    }

    // Fill date - use tomorrow's date
    const tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
    const dateStr = tomorrow.toISOString().split('T')[0]; // YYYY-MM-DD

    const dateInput = page.locator('input[type="date"], input[name="date"], [data-testid="show-date-input"]').first();
    ...
    console.log(`✅ Show "${showName}" verified in Supabase with ID: ${shows![0].id}`);
    console.log(`   Location: ${shows![0].location}`);
  });

  test('show date displays correctly (no off-by-one error)', async ({ page }) => {
    const errors = setupConsoleErrorTracking(page);
    const user = createTestUser();
    ...
    const showCard = showItem.locator('xpath=ancestor::*[contains(@class, "card") or contains(@class, "row") or @data-testid]').first();
    ...
    console.log(`✅ Show date displays correctly as ${expectedMonth} ${expectedDay}`);
    ...
    const editButton = page.locator('button:has-text("Edit"), [data-testid="edit-show-button"]').first();
    if (await editButton.isVisible()) {
      await editButton.click();
      await page.waitForTimeout(500);

      // Verify the date input still shows the correct date
      const editDateInput = page.locator('input[type="date"], input[name="date"]').first();
  ...
  test('duplicating a setlist syncs sourceSetlistId correctly', async ({ page }) => {
    const errors = setupConsoleErrorTracking(page);
    const user = createTestUser();
    ...
    const duplicateButton = page.locator('button:has-text("Duplicate"), button:has-text("Copy"), [data-testid="duplicate-setlist-button"]').first();
    ...
    if (await duplicateButton.isVisible()) {
      ...
      const { data: duplicates } = await supabase
        .from('setlists')
        .select('*')
        .or(`name.ilike.%copy%,name.ilike.%duplicate%,source_setlist_id.eq.${originalId}`);
