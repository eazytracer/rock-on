...
test.describe('Band Member Management', () => {
  ...
  test('admin can view all band members', async ({ browser }) => {
    ...
    await memberPage.waitForURL(/\/(songs|band-members|bands)/, { timeout: 10000 });
    ...
    const adminRow = adminPage.locator(`[data-testid="member-row-${admin.email}"]`).first();
    const memberRow = adminPage.locator(`[data-testid="member-row-${member.email}"]`).first();
  ...
  test('admin can promote member to admin', async ({ browser }) => {
    ...
    const promoteButton = adminPage.locator('button:has-text("Promote to Admin"), button:has-text("Make Admin")').first();
    ...
    if (hasPromoteButton) {
      ...
      // Verify role changed
      const updatedRole = memberRow.locator('[data-testid="member-role"]').first();
  ...
  test('admin can add instruments to member profile', async ({ browser }) => {
    ...
    const addInstrumentButton = adminPage.locator('button:has-text("Add Instrument"), [data-testid="add-instrument"]').first();
    const instrumentInput = adminPage.locator('input[name="instrument"], [data-testid="instrument-input"]').first();

    const hasAddButton = await addInstrumentButton.isVisible().catch(() => false);
    ...
    if (hasInput || hasAddButton) {
      ...
      const nameInput = adminPage.locator('input[placeholder*="Guitar"], input[name="instrumentName"]').first();
      const skillSelect = adminPage.locator('select[name="skill"], select[name="level"]').first();
      ...
      // Save
      const saveButton = adminPage.locator('button:has-text("Save"), button:has-text("Add")').first();
      const hasSave = await saveButton.isVisible().catch(() => false);
      if (hasSave) {
        await saveButton.click();
        await adminPage.waitForTimeout(1000);
  ...
  test('member can edit own profile', async ({ page }) => {
    ...
    const myRow = page.locator(`[data-testid="member-row-${user.email}"]`).first();
    ...
    const hasEditAccess = await page.locator('button:has-text("Add Instrument"), input[name="instrument"]').first().isVisible().catch(() => false);
  ...
  test('admin can remove member from band', async ({ browser }) => {
    ...
    const removeButton = adminPage.locator('button:has-text("Remove"), button:has-text("Remove from Band"), [data-testid="remove-member"]').first();
    ...
    if (hasRemoveButton) {
      ...
      const hasNoAccess = currentUrl.includes('get-started') ||
                         currentUrl.includes('auth') ||
                         await memberPage.locator('text=/no access|not found|no bands/i').first().isVisible().catch(() => false);
  ...
  test('regular member cannot remove other members', async ({ browser }) => {
    ...
    const member2Row = member1Page.locator(`[data-testid="member-row-${member2.email}"]`).first();
    ...
    if (isVisible) {
      ...
      const removeButton = member1Page.locator('button:has-text("Remove")').first();
  ...
  test('member count updates correctly when members join', async ({ browser }) => {
    const adminContext = await browser.newContext();
    const adminPage = await adminContext.newPage();
    ...
    const admin = createTestUser();
    await signUpViaUI(adminPage, admin);
    await createBandViaUI(adminPage, `Count Test ${Date.now()}`);

    // Check initial member count (should be 1)
    await adminPage.goto('/band-members');
    await adminPage.waitForLoadState('networkidle');

    const initialRows = await adminPage.locator('[data-testid^="member-row-"]').count();
    ...
    const updatedRows = await adminPage.locator('[data-testid^="member-row-"]').count();
