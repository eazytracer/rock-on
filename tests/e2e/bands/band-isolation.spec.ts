...
test.describe('Band Isolation and RLS Security', () => {
  ...
  test('users in different bands cannot see each others data', async ({ browser }) => {
    ...
    const addSongButton = page1.locator('button:has-text("Add Song"), [data-testid="add-song-button"]').first();
    ...
    if (hasAddButton) {
      ...
      // Fill song form
      const titleInput = page1.locator('input[name="title"], [data-testid="song-title-input"]').first();
      const artistInput = page1.locator('input[name="artist"], [data-testid="song-artist-input"]').first();
      ...
      if (hasTitleInput) {
        ...
        const saveButton = page1.locator('button:has-text("Save"), button[type="submit"]').first();
    ...
    const addSongButton2 = page2.locator('button:has-text("Add Song"), [data-testid="add-song-button"]').first();
    ...
    if (hasAddButton2) {
      ...
      const titleInput = page2.locator('input[name="title"], [data-testid="song-title-input"]').first();
      const artistInput = page2.locator('input[name="artist"], [data-testid="song-artist-input"]').first();
  ...
  test('user cannot access another bands members list', async ({ browser }) => {
    ...
    const user1Row = page1.locator(`[data-testid="member-row-${user1.email}"]`).first();
    await expect(user1Row).toBeVisible({ timeout: 5000 });

    // User 1 should NOT see User 2
    const user2Row = page1.locator(`[data-testid="member-row-${user2.email}"]`).first();
    ...
    const user2OwnRow = page2.locator(`[data-testid="member-row-${user2.email}"]`).first();
    await expect(user2OwnRow).toBeVisible({ timeout: 5000 });

    const user1RowInBandB = page2.locator(`[data-testid="member-row-${user1.email}"]`).first();
  ...
  test('RLS policies prevent unauthorized database access', async ({ browser }) => {
    const context1 = await browser.newContext();
    const page1 = await context1.newPage();
  ...
  test('switching bands shows correct isolated data', async ({ page }) => {
    ...
    if (hasSwitcher) {
      ...
      if (hasButton1) {
        await expect(page.locator('text=Band 1 Song').first()).toBeVisible({ timeout: 5000 });
      }
      ...
      // Should not see Band 1's song
      const stillCanSee = await page.locator('text=Band 1 Song').first().isVisible().catch(() => false);
  ...
  test('deleted band data is not accessible', async ({ browser }) => {
    ...
    console.log('Note: Band deletion test - feature may not be fully implemented');
  ...
  test('no data leaks through network requests', async ({ browser }) => {
    ...
    console.log(`Tracked ${dataRequests.length} data requests - no unauthorized data detected`);
