...
export async function signUpViaUI(page: Page, user: TestUser): Promise<void> {
  ...
  await page.waitForSelector('button:has-text("Log In"), button:has-text("Create Account")', {
    state: 'visible',
    timeout: 10000
  });
  ...
  const confirmPasswordField = page.locator('[data-testid="signup-confirm-password-input"]');
...
export async function loginViaUI(page: Page, user: TestUser): Promise<void> {
  ...
  const hasSignUpButton = await page.locator('button:has-text("Sign Up"), button:has-text("Create Account")').isVisible();
  if (hasSignUpButton) {
    const switchToLogin = page.locator('button:has-text("Sign In"), a:has-text("Sign In")');
...
export async function logoutViaUI(page: Page): Promise<void> {
  ...
  const userMenu = page.locator('[data-testid="user-menu"], button:has-text("Log Out")');

  // If there's a user menu, click it first
  const hasUserMenu = await page.locator('[data-testid="user-menu"]').isVisible();
  if (hasUserMenu) {
