...
import { SongCasting, SongAssignment, AssignmentRole } from '../../models/SongCasting'
...
export const CastingComparison: React.FC<CastingComparisonProps> = ({
  ...
  const loadComparisonData = async () => {
    ...
    try {
      ...
      const castings1 = await castingService.getCastingsForContext(context1.type, context1.id)
      const castings2 = await castingService.getCastingsForContext(context2.type, context2.id)
  ...
  const loadCastingData = async (casting?: SongCasting): Promise<SongCastingData | null> => {
    ...
    try {
      ...
      const assignmentsWithRoles = complete.assignments.map((assignment: any) => {
  ...
  const checkForDifferences = (
    ...
    const memberIds1 = new Set(data1!.assignments.map(a => a.assignment.memberId))
    const memberIds2 = new Set(data2!.assignments.map(a => a.assignment.memberId))
  ...
  const handleCopyCasting = async (fromContext: Context, toContext: Context, songId: number) => {
    ...
    try {
      ...
      const sourceCastings = await castingService.getCastingsForContext(fromContext.type, fromContext.id)
      ...
      alert('Copying individual song casting is not yet fully implemented. Use "Copy All" instead.')
      return
  ...
  const handleCopyAllCasting = async (fromContext: Context, toContext: Context) => {
    const confirmed = window.confirm(
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-900">Compare Casting</h2>
          {onClose && (
            <button
              onClick={onClose}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              value={context1?.id || ''}
              onChange={(e) => {
                const selected = availableContexts.find(c => c.id === e.target.value)
            ...
            >
              ...
              {availableContexts.map(context => (
                <option key={context.id} value={context.id} disabled={context.id === context2?.id}>
          ...
          <div>
            ...
            >
              <option value="">-- Select context --</option>
              {availableContexts.map(context => (
                <option key={context.id} value={context.id} disabled={context.id === context1?.id}>
                  {context.name} ({context.type})
                </option>
              ))}
            </select>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-blue-900">
                      Comparing {comparisonData.length} song{comparisonData.length !== 1 ? 's' : ''}
                    </div>
                    <div className="text-xs text-blue-700 mt-1">
                      {getDifferenceCount()} difference{getDifferenceCount() !== 1 ? 's' : ''} found
                    </div>
                  </div>
              ...
              {comparisonData.map(comparison => (
                <div
                  ...
                  className={`border rounded-lg overflow-hidden ${
                    comparison.hasDifferences ? 'border-yellow-300 bg-yellow-50' : 'border-gray-200'
                  }`}
                >
                  {/* Song Header */}
                  <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-medium text-gray-900">{comparison.songTitle}</h4>
                        <p className="text-sm text-gray-600">{comparison.songArtist}</p>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2">
                    ...
                    <div className="p-4 border-b md:border-b-0 md:border-r border-gray-200">
                      <h5 className="text-sm font-semibold text-gray-700 mb-2">{context1.name}</h5>
                      {comparison.context1Data ? (
                        <div className="space-y-2">
                          {comparison.context1Data.assignments.map(({ assignment, roles, memberName }) => (
                        ...
                        <p className="text-sm text-gray-500 italic">No casting</p>
                      )}
                      {comparison.context1Data && comparison.hasDifferences && (
                        <TouchButton
                          variant="ghost"
                          size="sm"
                          onClick={() => handleCopyCasting(context1, context2, comparison.songId)}
                          loading={copying}
                          className="mt-3"
                        >
                    ...
                    {/* Context 2 */}
                    <div className="p-4">
                      <h5 className="text-sm font-semibold text-gray-700 mb-2">{context2.name}</h5>
                      {comparison.context2Data ? (
                        <div className="space-y-2">
                          {comparison.context2Data.assignments.map(({ assignment, roles, memberName }) => (
                            <div key={assignment.id} className="text-sm">
                              <div className="font-medium text-gray-900">{memberName}</div>
                              <div className="text-gray-600 ml-2">
                                {roles.map(r => r.name).join(', ')}
                      ...
                      {comparison.context2Data && comparison.hasDifferences && (
                        <TouchButton
                          variant="ghost"
                          size="sm"
                          onClick={() => handleCopyCasting(context2, context1, comparison.songId)}
                          loading={copying}
                          className="mt-3"
                        >
