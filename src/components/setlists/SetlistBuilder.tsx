...
export const SetlistBuilder: React.FC<SetlistBuilderProps> = ({
  ...
  const [colorCodeField, setColorCodeField] = useState<
    'guitarTuning' | 'key' | 'difficulty' | 'readiness'
  >('guitarTuning')
  ...
  const hasChanges = React.useMemo(() => {
    ...
    const songsChanged = setlistSongs.some((song, index) => {
      ...
      return (
        !originalSong ||
        song.songId !== originalSong.songId ||
        song.order !== originalSong.order
      )
    })
  ...
  const handleTouchStartWrapper = useCallback(
    (e: React.TouchEvent, index: number, fromAvailable: boolean = false) => {
    ...
    (e: TouchEvent) => {
      ...
      if (touchStartRef.current) {
        ...
        if (deltaY > 5 && deltaTime > 50) {
          ...
          const dropIndex = setlistElement
            ? parseInt(setlistElement.getAttribute('data-index') || '-1')
            : null
    ...
    (e: TouchEvent) => {
      ...
      if (setlistElement) {
        const dropIndex = parseInt(
          setlistElement.getAttribute('data-index') || '0'
        )
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent, index: number, fromAvailable: boolean = false) => {
      if (!reorderMode && !fromAvailable) return
  ...
  const getColorByField = (songId: string) => {
    ...
    switch (colorCodeField) {
      ...
      case 'difficulty': {
        ...
        return (
          difficultyColors[song.difficulty - 1] || 'bg-gray-100 border-gray-300'
        )
      }
      ...
      case 'key': {
        ...
        const uniqueKeys = [
          ...new Set(
            setlistSongs.map(ss => getSongById(ss.songId)?.key).filter(Boolean)
          ),
        ]
        const keyIndex = uniqueKeys.indexOf(song.key)
        return keyIndex >= 0
          ? colorPalette[keyIndex % colorPalette.length]
      ...
      default: {
        ...
        return tuningIndex >= 0
          ? colorPalette[tuningIndex % colorPalette.length]
          : 'bg-gray-100 border-gray-300'
      }
    }
  }
                    ...
                    <>
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          ...
                          onChange={e =>
                            setColorCodeField(
                              e.target.value as typeof colorCodeField
                            )
                          }
              ...
              >
                {setlistSongs.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p>
                      Click songs from the right to add them to your setlist
                    </p>
                  </div>
                ) : (
