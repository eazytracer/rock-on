...
export const LinkedSongView: React.FC<LinkedSongViewProps> = ({
  ...
  const getRelationshipBadge = (relationship: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${colors[relationship] || colors.variant}`}>
    ...
    <div className={`bg-white rounded-lg border border-gray-200 ${className}`}>
      ...
      <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-sm font-medium text-gray-900">Linked Variants: {songGroup.name}</h3>
          <span className="text-xs text-gray-500">({linkedSongs.length} versions)</span>
        </div>
      ...
      <div className="divide-y divide-gray-200">
        {linkedSongs.map((song) => {
          ...
          const canContribute = song.contextType === 'personal' && onContributeToBand
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">by {song.artist}</p>

                  {song.notes && (
                    <p className="text-xs text-gray-600 mt-2 italic">{song.notes}</p>
                  )}
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                    <div>
                      ...
                      <span className="text-gray-900 font-medium">{song.key}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">BPM:</span>{' '}
                      <span className="text-gray-900 font-medium">{song.bpm}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Difficulty:</span>{' '}
                      <span className="text-gray-900 font-medium">{song.difficulty}/5</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Confidence:</span>{' '}
                      <span className="text-gray-900 font-medium">{song.confidenceLevel}/5</span>
                    </div>
                  </div>
                  ...
                  {song.guitarTuning && (
                    <div className="mt-2 text-xs">
                      ...
                      <span className="text-gray-900 font-mono">{song.guitarTuning}</span>
                    </div>
                  )}
                </div>
