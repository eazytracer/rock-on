...
export const SongLinkingSuggestions: React.FC<SongLinkingSuggestionsProps> = ({
  ...
  const [linkingInProgress, setLinkingInProgress] = useState<Set<string>>(
    new Set()
  )
  ...
  const getConfidenceBadgeColor = (
    confidence: LinkingSuggestion['confidence']
  ) => {
    switch (confidence) {
      case 'high':
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center space-x-2 mb-2">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getConfidenceBadgeColor(suggestion.confidence)}`}
                    >
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center space-x-2 text-sm">
                      ...
                      <span className="text-xs text-gray-400">
                        ({getContextLabel(suggestion.song)})
                      </span>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 text-sm">
                      ...
                      <span className="font-medium text-gray-900">
                        {suggestion.targetSong.title}
                      </span>
                      <span className="text-gray-500">
                        by {suggestion.targetSong.artist}
                      </span>
                      <span className="text-xs text-gray-400">
                        ({getContextLabel(suggestion.targetSong)})
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2 ml-4">
                  ...
                  <button
                    onClick={() =>
                      onDismiss(suggestion.song.id!, suggestion.targetSong.id!)
                    }
                    disabled={isLinking}
