...
export class SyncService {
  ...
  async triggerSync(): Promise<boolean> {
    ...
    console.log(
      `[SyncService] Starting sync for ${this.syncQueue.length} pending changes`
    )
    ...
    try {
        ...
        try {
          ...
          if (success) {
            ...
            if (change.attempts < this.maxRetries) {
              ...
              console.error(
                '[SyncService] Change exceeded max retries:',
                change
              )
            }
          }
      ...
      if (successful.length > 0) {
        ...
        console.log(
          `[SyncService] Successfully synced ${successful.length} changes`
        )
      }
  ...
  private async syncChange(change: PendingChange): Promise<boolean> {
    ...
    console.log(
      `[SyncService] Syncing ${change.action} ${change.type}:`,
      change.data.title || change.data.name || change.id
    )
    ...
    try {
      ...
      console.error(
        `[SyncService] Validation failed for ${change.type}:`,
        error
      )
      return false
    }
  ...
  private async validateSessionData(change: PendingChange): Promise<boolean> {
    ...
    if (!data.scheduledDate || !data.bandId) {
      console.error(
        '[SyncService] Invalid session data - missing required fields'
      )
      return false
    }
  ...
  private async validateSetlistData(change: PendingChange): Promise<boolean> {
    ...
    if (!data.name || !data.bandId) {
      console.error(
        '[SyncService] Invalid setlist data - missing required fields'
      )
      return false
    }
  ...
  private async validateMemberData(change: PendingChange): Promise<boolean> {
    ...
    if (!data.name || !data.email) {
      console.error(
        '[SyncService] Invalid member data - missing required fields'
      )
      return false
    }
  ...
  private async loadPendingChanges(): Promise<void> {
    try {
      ...
      if (stored) {
        ...
        console.log(
          `[SyncService] Loaded ${this.syncQueue.length} pending changes`
        )
      }
    } catch (error) {
