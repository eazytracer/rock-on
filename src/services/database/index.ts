...
export class RockOnDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      songs:
        '++id, title, artist, key, difficulty, createdDate, lastPracticed, confidenceLevel',
      practiceSessions: '++id, bandId, scheduledDate, type, status',
      setlists:
        '++id, name, bandId, showDate, status, createdDate, lastModified',
    })
    ...
    this.version(2).stores({
      ...
      bandMemberships:
        '++id, userId, bandId, role, joinedDate, status, *permissions',
    ...
    this.version(8)
      .stores({
        ...
        songs:
          '++id, title, artist, key, difficulty, createdDate, lastPracticed, confidenceLevel, contextType, contextId, createdBy, visibility, songGroupId',
        practiceSessions:
          '++id, bandId, scheduledDate, type, status, setlistId',
        setlists:
          '++id, name, bandId, showId, status, createdDate, lastModified',
        shows: '++id, bandId, setlistId, scheduledDate, status, venue',
        users: '++id, email, name, createdDate, lastLogin, authProvider',
        userProfiles:
          '++id, userId, displayName, primaryInstrument, *instruments',
        bandMemberships:
          '++id, [userId+bandId], userId, bandId, role, joinedDate, status, *permissions', // FIXED: Added compound unique index
        inviteCodes: '++id, bandId, code, createdBy, expiresAt, currentUses',
        songGroups: '++id, createdBy, name, createdDate',
        songGroupMemberships: '++id, songId, songGroupId, addedBy, addedDate',
        songCastings:
      ...
      .upgrade(async tx => {
        ...
        console.log(
          '[DB Migration v8] Cleaning up duplicate band memberships...'
        )
        ...
        for (const [_key, dupes] of grouped) {
          if (dupes.length > 1) {
            ...
            dupes.sort((a, b) => {
              const dateA =
                a.joinedDate instanceof Date
                  ? a.joinedDate
                  : new Date(a.joinedDate)
              const dateB =
                b.joinedDate instanceof Date
                  ? b.joinedDate
        ...
        if (duplicatesRemoved > 0) {
          console.log(
            `[DB Migration v8] Removed ${duplicatesRemoved} duplicate memberships`
          )
        } else {
    ...
    this.songGroupMemberships.hook(
      'creating',
      function (_primKey, obj, _trans) {
