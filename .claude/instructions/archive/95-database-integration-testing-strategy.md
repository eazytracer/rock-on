...
import {
  ...
  createTestUsers,
} from '../setup/supabase-test-instance'
...
describe('RLS Policies - Songs', () => {
  ...
  describe('INSERT policy', () => {
    ...
    it('outsiders cannot create songs for band', async () => {
      const { data, error } = await outsiderClient.from('songs').insert({
...
describe('Sync Integration - Shows', () => {
  it('should create show via ShowService', async () => {
    const show = await ShowService.createShow({
      ...
      duration: 120,
      status: 'scheduled',
      contacts: [{ id: '1', name: 'Manager', role: 'Venue Manager' }],
    })
...
describe('Data Migration Validation', () => {
  it('migrates shows from practice_sessions correctly', async () => {
    ...
    const practices = await instance.select('practice_sessions', {
      type: 'gig',
    })
...
```json
{
  "tables": {
    "songs": {
      ...
      "indexes": ["idx_songs_context", "idx_songs_created_by"],
      "policies": [
