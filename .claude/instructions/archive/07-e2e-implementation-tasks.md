...
2. Implement test flow:
   ...
   test.describe('User Signup and Band Creation', () => {
     ...
     test('new user can sign up and create first band', async ({ page }) => {
       ...
       await expect(page.locator('[data-testid="band-name"]')).toHaveText(
         bandName
       )
...
2. Implement multi-user test flow:
   ...
   ```typescript
   test('new user can join existing band via invite code', async ({
     browser,
   }) => {
     ...
     await expect(page2.locator('[data-testid="band-name"]')).toHaveText(
       bandName
     )

     // Verify User 1 sees User 2 in members list
     await page1.goto('/band-members')
     const memberCount = await page1
     ...
     const user2Role = page2.locator(
       `[data-testid="member-role"][data-email="${user2.email}"]`
     )
...
2. Implement add song test:
   ...
   test('member can add song to band', async ({ page }) => {
     ...
     const songRow = page
       ...
       .filter({ hasText: 'Stairway to Heaven' })
...
2. Implement isolation test:
   ...
   test('users can only see their own band data', async ({ browser }) => {
     ...
     const bandASong = page2
       .locator('[data-testid^="song-row-"]')
       .filter({ hasText: 'Band A Song' })
